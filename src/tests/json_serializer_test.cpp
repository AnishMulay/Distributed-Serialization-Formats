...
void test_metadata_serialization_deserialization() {
    ...
    std::cout << "Serialized metadata (JSON): " << std::string(serialized.begin(), serialized.end()) << std::endl;
...
void test_block_serialization_deserialization() {
    ...
    for (size_t size : sizes) {
        ...
        std::cout << "Serialized block of size " << size << " bytes to " 
                  << serialized.size() << " bytes (JSON)" << std::endl;
...
int main() {
    std::cout << "Running JSON serializer tests..." << std::endl;
